...
class TestAzdValidator(unittest.TestCase):
    ...
    @patch("validator.azd_validator.AzdValidator.list_resources")
    @patch("subprocess.run")
    def test_validate_retry_logic_finally_failed(
        self, mock_runCommand, mock_list_resources
    ):
        # Simulate a retryable error
        mock_runCommand.side_effect = [
            subprocess.CalledProcessError(
    ...
    def test_list_resources(self, mock_run, mock_list_resources):
        ...
        mock_list_resources.assert_called_once_with(
            "mocked-resource-group", "mocked-subscription-id"
        )

        # Check the result of the list_resources method
        expected_result = indent(
            "List of all resource types in the resource group mocked-resource-group:\nresource1\nresource2\n List of all AI deployments:\ndeployment1\ndeployment2",
            2,
        )
