...
def test_file_validator_file_found_exist():
    validator = FileValidator("TestCatalog", "LICENSE", [""], "test/data", ["."], None, False)
...
def test_file_validator_file_not_found_case_sensitive():
    validator = FileValidator("TestCatalog", "license", [""], "test/data", ["."], None, True)
    result, message = validator.validate()
    assert result == False
    assert message == ItemResultFormat.FAIL.format(message="license File", detail_messages=ItemResultFormat.SUBITEM.format(sign=Signs.BLOCK, message="Error: license file is missing."))
...
def test_file_validator_file_found_mutiple_extension():
    validator = FileValidator("TestCatalog", "LICENSE", ["md", "txt", ""], "test/data", ["."], None, False)
    result, message = validator.validate()
    assert result == True
...
def test_file_validator_file_not_found_mutiple_extension():
    ...
    result, message = validator.validate()
    assert result == False
    assert message == ItemResultFormat.FAIL.format(message="LICENSE.md File", detail_messages=ItemResultFormat.SUBITEM.format(sign=Signs.BLOCK, message="Error: LICENSE.md file is missing."))
...
def test_file_validator_H2Tag_found_case_non_sensitive():
    validator = FileValidator("TestCatalog", "README", ["md"], "test/data", ["."], ["## Test h2"], False)
    result, message = validator.validate()
    assert result == True
    assert message == ItemResultFormat.PASS.format(message="README.md File")

def test_file_validator_H2Tag_found_case_sensitive():
    validator = FileValidator("TestCatalog", "README", ["md"], "test/data", ["."], ["## Test H2"], True)
    result, message = validator.validate()
    assert result == True
    assert message == ItemResultFormat.PASS.format(message="README.md File")

def test_file_validator_H2Tag_not_found_case_non_sensitive():
    validator = FileValidator("TestCatalog", "README", ["md"], "test/data", ["."], ["## Test h2 Not Found"], False)
    result, message = validator.validate()
    assert result == False
    assert message == ItemResultFormat.FAIL.format(message="README.md File", detail_messages=ItemResultFormat.SUBITEM.format(sign=Signs.BLOCK, message="Error: ## Test h2 Not Found is missing in README.md."))

def test_file_validator_H2Tag_not_found_case_sensitive():
    validator = FileValidator("TestCatalog", "README", ["md"], "test/data", ["."], ["## Test H2 Not Found"], True)
    result, message = validator.validate()
    assert result == False
    assert message == ItemResultFormat.FAIL.format(message="README.md File", detail_messages=ItemResultFormat.SUBITEM.format(sign=Signs.BLOCK, message="Error: ## Test H2 Not Found is missing in README.md."))
...
def test_file_validator_folder_allowed_but_not_exists():
    ...
    result, message = validator.validate()
    assert result == False
    assert message == ItemResultFormat.FAIL.format(message="NON_EXISTENT_FOLDER.md File or NON_EXISTENT_FOLDER Folder", detail_messages=ItemResultFormat.SUBITEM.format(sign=Signs.BLOCK, message="Error: NON_EXISTENT_FOLDER.md file or NON_EXISTENT_FOLDER folder is missing."))
