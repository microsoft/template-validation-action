    ...
    def parse(self):
        ...
        for rule_name, rule_details in rules.items():
            ...
            if validator_type == "FileValidator":
                ...
                playwright_config_ts_paths = utils.find_playwright_config_ts_path(self.args.repo_path)
                logging.debug(f"playwright_config_ts_paths: {playwright_config_ts_paths}")

                for playwright_config_ts_path in playwright_config_ts_paths:
                    validator = PlaywrightTestValidator(catalog, playwright_config_ts_path, severity)
                    validators.append(validator)
            else:
                continue
