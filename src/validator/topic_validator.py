...
class TopicValidator(ValidatorBase):
    def __init__(self, catalog, name, expected_topics, actual_topics, error_as_warning=False):
    ...
    def validate(self):
        ...
        if result:
            ...
            messages.append(ItemResultFormat.WARNING.format(
                message=f"Topics on repo contains {self.expected_topics}", detail_messages=line_delimiter.join(subMessages)))
