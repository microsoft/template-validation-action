...
class FolderValidator(ValidatorBase):
    def __init__(self, validatorCatalog, folderName, candidatePaths, errorAsWarning=False):
        super().__init__(f"{folderName}FolderValidator", validatorCatalog, errorAsWarning)
    ...
    def validate(self):
        ...
        if not folder_found:
            messages.append(ItemResultFormat.FAIL.format(
                message=f"{self.folderName} Folder", detail_messages=ItemResultFormat.SUBITEM.format(sign=Signs.BLOCK, message=f"Error: {self.folderName} folder is missing in all candidate folders.")))
            result = False
        else:
            result = True
